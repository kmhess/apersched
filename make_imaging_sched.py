...
def write_to_parset(writer,i,source_name,source_pos,start_datetime,end_datetime):
...
def observe_target(obstimeUTC, name, obstime=11.5):
    # Use name to subtract 1 value from the weights column!
    apertif_fields['weights'][apertif_fields['name'] == name] -= 1
...
# Retrieve names of observations from ATDB (excluding calibrator scans)
...
total_wait = 0
# Open & prepare CSV file to write parset parameters to, in the same format as a csv file from Google Sheets.
...
print("Total wait time: {} mins is {:3.1f}% of total.".format(total_wait, \
                                                      total_wait*60./(obstimeUTC-starttimeUTC).total_seconds()*100))
